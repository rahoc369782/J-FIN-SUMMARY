...
print_title "JERENS Finance Report - $current_date" >> "$REPORT_FILE"
...
print_sum_title "Net Worth: $(format_number "$current_net_worth") (($net_worth_change)% change)" >> "$REPORT_FILE"

# Account Balances Section
print_title "Account Balances" >> "$REPORT_FILE"
printf "%-35s %15s %15s\n" "Account" "Balance" "Change (%)" >> "$REPORT_FILE"
printf "%-35s %15s %15s\n" "-----------------------------------" "---------------" "---------------" >> "$REPORT_FILE"
...
print_title "Investment Details" >> "$REPORT_FILE"
printf "%-35s %15s %15s\n" "Investment Account" "Balance" "Change (%)" >> "$REPORT_FILE"
printf "%-35s %15s %15s\n" "-----------------------------------" "---------------" "---------------" >> "$REPORT_FILE"
...
print_title "Expense's Report" >> "$REPORT_FILE"
...
git add "$REPORT_FILE"
git commit -m "Add finance report for $current_date"
